...
jq -e "map(select(.isDraft==false and .isPrerelease==false and .tagName==\"$(printf "$1" | b64jq .file | awk -F '_' '{print $2}')/$(printf $1 | b64jq -r .channel)/$(printf $1 | b64jq -r .chrome_version)\"))==[]" >/dev/null && \
